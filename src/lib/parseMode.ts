// Extracted from parse-mode for grammy.
...
const buildTransformer = (parseMode: string) => {
    const normalizedParseMode = wellKnownParseModesMap.get(parseMode.toLowerCase()) ?? parseMode;
        ...
        switch (method) {
            case "editMessageMedia":
                if ("media" in payload && !("parse_mode" in payload.media)) {
                    // @ts-ignore
                    payload.media.parse_mode = normalizedParseMode;
                }
                break;
                ...
                if ("results" in payload) {
                    for (const result of payload.results) {
                        if (
                            ...
                            // @ts-ignore
                            result.input_message_content.parse_mode = normalizedParseMode;
                        } else if (!("parse_mode" in result)) {
                            // @ts-ignore
                            result.parse_mode = normalizedParseMode;
                        }
                    }
                }
                break;
            ...
            default:
                payload = { ...payload, ...{ parse_mode: normalizedParseMode } };
        }
